//Done
/*
    use suffix array to compute the answer
    step 1: - get the diff at i and i + 1 and mod it to 8
                add 8 if it's below 0;
            - append to the answer string.
            - return the answer string.
    step 2: - create a string and append the normalized number from step 1 twice
            - construct suffix array using Howard Cheng's code
            - iterate through the suffix array
                - if the size of the suffix array at index i is the same as the input.size()
                    - iterate through that element and print the answer.

    constructing the suffix array will sort the element lexicographically.
    As long the size is the same in the suffix array, then that's the best answer.
    The suffix array is sorted using radix sort.
*/
 ...
 *
 *   J. K�rkk�inen and P. Sanders. Simple linear work suffix array
...
string normalizeStep1(const string &input);
string normalizeStep2(const string &input);

int main()
{
    string input;
    while(cin >> input && input.size())
    {
        string normalize1 = normalizeStep1(input);
        string normalize2 = normalizeStep2(normalize1);
...
string normalizeStep1(const string &input)
{
    ...
    {
        int num1 = (int)input[i % input.size()];
        int num2 = (int)input[(i + 1) % input.size()];
        int diff = (num2 - num1) % 8; if(diff < 0){diff += 8;}
        normal += diff + '0';   //an offset to get the numbers from ASCII
    }
...
string normalizeStep2(const string &input)
{
    int suffix [300001 * 2];    // 300001 because the max length is 300,000.
                                // x2 because suffix array will contain the
                                // 'input' string appended twice. This will be
                                // used as a string rotation
    string normal = input + input;
    build_sarray(normal, suffix);
    string answer = "";

    for(int i = 0; i < input.size() * 2; i++)
    {
        //if the answer is the same size as the input.size()
        if(suffix[i] < input.size())
        {
            //get the answer
            for(int k = 0; k < input.size(); k++)
            {
                answer += normal[suffix[i] + k];
            }
            return answer;
        }
    }
