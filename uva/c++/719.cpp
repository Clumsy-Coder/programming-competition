// DONE
/*
	Build the suffix array using Howard Cheng's code
	
	read the string 'text'
	create a new string and set the 'text' twice
	build the suffix array
	go through the suffix array
	if the size is <= 'text' size
		print the answer + 1
...
int main()
{
	...
	while(cases--)
	{
		...
		str += '|';					// removes the need to check lcp.
									// since | is always the last char
		...
		build_sarray(str, suffix);	// build the suffix array

		for(int i = 0; i < str.size(); i++)
		{
			if(suffix[i] <= text.size())
			{
				cout << suffix[i] + 1 << endl;
				break;
			}
		}
	}
