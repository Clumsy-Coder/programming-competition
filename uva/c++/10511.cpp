//DONE
/*
	Use network flow to solve the problem
	there are three main nodes. Party, person and club
	Generate the graph using Howard's code and generate the map.
*/
...
long long counter = 3;		//0 is the source, 1 is the sink
const int SOURCE = 1, SINK = 2;
map<int, string> intNodes;	//storing a number to a name, club, part.
map<string, int> nameMap, partyMap, clubMap;
set<string> partyV, clubsV;
	...
	while(cases--)
	{
		...
		while(getline(cin, temp) && temp.size())
		{
			...
			while(ss >> club)
			{
				...
				{
					clubMap[club] = counter;
					intNodes[counter] = club;
					counter++;
				}
				clubsV.insert(club);
				G.add_edge(nameMap[name], clubMap[club], 1);
			}
		}

		int sourceWeight = (clubsV.size() - 1 )/ 2;
		for(auto i = partyV.begin(); i != partyV.end(); i++)
		{
			G.add_edge(SOURCE, partyMap[*i], sourceWeight);
		}
		for(auto i = clubsV.begin(); i != clubsV.end(); i++)
		{
			G.add_edge(clubMap[*i], SINK, 1);
		}

		flow = network_flow(G, SOURCE, SINK);

		if(flow == clubsV.size())
		{
			for(auto it = nameMap.begin(); it != nameMap.end(); it++)
				for(auto i = G.nbr[it->second].begin(); i != G.nbr[it->second].end(); i++)
					if(i->flow == 1 && i->is_real)
					{
						cout << it->first << " " << intNodes[i->to] << endl;
						break;
					}
		}
		else
