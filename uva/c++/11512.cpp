...
/*
    Solve using suffix array and lcp
*/
...
int main()
{
    ...
    while(cases--)
    {
        ...
        int suffix [1001], lcp [1001];
        string str = text;
        build_sarray(str, suffix);
        compute_lcp(str, suffix, lcp);
        ...
        if(maxLcp != 0)
        {
            //because it must have at least 2 repeations
            int counter = 2;        
            // count the occurences of the prefix
            // make sure the current prefix is >= largest prefix found
            for (int i = maxI + 1; (i < text.size()) && (lcp[i] >= maxLcp); i++)
                counter++;

            // append the parts of the prefix.
            string answer = "";
            for (int i = 0; i < maxLcp; i++)
                answer += text[suffix[maxI] + i];
